...
import { useAccount, useNetwork, useSwitchNetwork, useWriteContract, useReadContract, useWaitForTransactionReceipt } from "wagmi";
...
export function setupPresaleFunctions() {
  ...
  function useBuyTokens() {
    ...
    const buyTokens = useCallback(
      ...
      [isConnected, chain, writeContract, address, switchNetwork]
    );

    return { buyTokens, isLoading: isLoading || isTxPending || isConfirming, error: error || writeError, txHash: txData };
  }
  ...
  function useTokenBalance(tokenType) {
    ...
    return { balance, isLoading: isLoading || isBalanceLoading };
  }
